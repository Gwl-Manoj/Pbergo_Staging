 ...
 * @Project: Bluetoad
 * @AutomationEngineer: Ravish Soni, Vasu, Versha
 * @Date: June 27, 2022
 * */
import java.io.*;
...
public class CustomizedReport implements IReporter {
	...
	private static final String emailableReportTemplateFile = System.getProperty("user.dir")
			+ "\\src\\main\\java\\com\\pbergo\\base\\CustomizedReportTemplate.html";
	...
	//
		...
		try {
			...
			String customReportTitle = this.getCustomReportTitle("PBErgo WebApp Report");
	...
	@SuppressWarnings({ "finally", "static-access", "resource" })
		...
		try {
			...
			for (ISuite tempSuite : suites) {
				...
				for (ISuiteResult result : testResults.values()) {
					...
					/* Get browser type. */
					String platformName = tempSuite.getParameter("platformName");
